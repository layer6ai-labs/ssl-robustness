# Adapted from https://github.com/facebookresearch/dinov2/blob/main/dinov2/eval/depth/models/depther/encoder_decoder.py
...
from models.downstream_models.depther.depth_head import resize
from models.downstream_models.depther.base_depther import BaseDepther
...
class DepthEncoderDecoder(BaseDepther):
    ...
    EncoderDecoder typically consists of encoder, (neck) and decode_head.
    """
    ...
    def __init__(
        ...
        if pretrained is not None:
            assert (
                encoder.get("pretrained") is None
            ), "both encoder and depther set pretrained weight"
    ...
    def extract_feat(self, img):
        ...
        img = self.add_pad(img)
        x = self.encoder(img)
        return x

    def get_representation(self, x):
        return self.extract_feat(x)[0][0]

    def encode_decode(self, img, img_metas, rescale=True, size=None):
        """Encode images with encoder and decode into a depth estimation
    ...
    def _get_pad(self, size, multiple=14):
    ...
    def _decode_head_forward_train(self, x, depth_gt, **kwargs):
        """Run forward function and calculate loss for decode head in
        training."""
        losses = dict()
        loss_decode = self.decode_head.forward_train(x, depth_gt)
    ...
    def forward_train(self, img, img_metas, depth_gt, **kwargs):
        ...
        loss_decode = self._decode_head_forward_train(x, depth_gt, **kwargs)
